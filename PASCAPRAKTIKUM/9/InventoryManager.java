...
public class InventoryManager<A extends Barang> {
    ...
    public void addCatalog(A item, int jumlah) {
        // Cek apakah item sudah ada dalam inventory
        for (CatalogBarang<A> catalog : inventory) {
            if (catalog.barang().getName().equals(item.getName())) {
                // Jika ada, update jumlahnya (buat objek baru, ganti di posisi yang sama)
                int idx = inventory.indexOf(catalog);
                CatalogBarang<A> updated = new CatalogBarang<>(item, catalog.jumlah() + jumlah);
                inventory.set(idx, updated);
                return;
            }
        }
        // Jika tidak ada, tambahkan item baru ke inventory
    ...
    public void removeCatalog() {
        if (inventory.isEmpty()) {
            ...
            // Menghapus katalog barang terakhir
    ...
    public int getTotalAmount(String itemType) {
        ...
        for (CatalogBarang<A> catalog : inventory) {
            if ((itemType.equals("Baju") && catalog.barang() instanceof Baju) ||
                (itemType.equals("Handphone") && catalog.barang() instanceof Handphone)) {
    ...
    public void printInventory() {
        if (inventory.isEmpty()) {
            ...
            for (CatalogBarang<A> catalog : inventory) {
                Barang barang = catalog.barang();
                System.out.println("[" + barang.getName() + "(" + barang.getPrice() + ") : " + catalog.jumlah() + "]");
            }
        }
    }
