...
public class Monster implements MonsterTarget, Comparable<Monster> {
    ...
    public void checkEnrage() {
        if(!isEnraged() && health>0 && health<enragedThreshold){
            isEnraged = true;
        }
    }
    ...
    public void counterAttack(Hunter hunter) {
        double dmgMultiplier = isEnraged ? ENRAGE_MULTIPLIER : 1;
        int damage = (int)(attackPower*dmgMultiplier);
    ...
    public void areaAttack(Hunter[] hunters) {
        double dmgMultiplier = isEnraged ? ENRAGE_MULTIPLIER : 1;
        int damage = (int)(attackPower*0.7*dmgMultiplier);
